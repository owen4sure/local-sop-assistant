...
| 領域 | 做什麼 |
|---|---|
| 🔍 **SOP 問答** | 用自然語言問整個知識庫——網頁聊天與 LINE 都行。混合檢索(向量+關鍵字)扛得住口語贅字、標點差異和專有名詞。每個回答都附即時信心徽章(語意搜尋命中幾份文件,點下去看各文件相關度%),旁邊還有面板顯示回答實際引用的原文段落。 |
| 🧰 **個人工作區** | 問答頁同時是個輕量個人儀表板:筆記、行程/待辦分頁、通訊錄、常用連結——外加問 SOP 以外事情的**自由聊天模式**。所有個人工具資料都存在瀏覽器自己的 local storage,不會送到、也不會存在伺服器上。 |
| 📂 **對話內文件管理** | 直接瀏覽目前的文件清單、開啟任一來源文件,或叫出跟審核台同一套的原地編輯器——送審或(輸入審核密碼)直接更新。也有申請刪除與直接刪除兩種流程。 |
