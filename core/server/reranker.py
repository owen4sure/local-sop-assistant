#!/usr/bin/env python3
# 本機 reranker 小服務：cross-encoder（多語 XLM-RoBERTa 系，例 bge-reranker-v2-m3）。只用 torch + transformers。
# 用途：把混合檢索的候選 chunk 對 query 做精排，分數可校準「查無資料」門檻。
# 介面：POST /rerank {query, documents:[str]} -> {scores:[float 0..1]}；GET /health
...
MODEL_ID = os.environ.get("RERANKER_MODEL", "<RERANKER_MODEL_ID>")  # 例 BAAI/bge-reranker-v2-m3
PORT = int(os.environ.get("RERANKER_PORT", "<RERANK_PORT>"))
...
def rerank(query, documents):
    ...
    return torch.sigmoid(logits).cpu().tolist()  # 0..1
    ...
    def do_POST(self):
        ...
        try:
            ...
            with _lock:  # cross-encoder 非執行緒安全，序列化
                scores = rerank(data.get("query", ""), data.get("documents", []))
